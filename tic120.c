...
/*
 * The address of TIC120 display with PCF8531 controller.
 */
#define TIC120_ADDRESS                      0x78

/*
 * The command to transmit one command byte to the PCF8531 controller.
 */
#define CON1                                0x80

/*
 * The command to transmit some data bytes to the PCF8531 controller.
 */
#define CON2                                0x40

/*
 * Code page in ASCII format.
 */
...
/**
 * @brief  Waits a little bit.
 */
void delay_little_bit() {
    for (u8 i=0; i<5; i++) {
    }
}
...
void i2c_init() {
    ...
    I2C_InitStructure.I2C_OwnAddress1 = TIC120_ADDRESS;
...
/**
 * @brief   Checks I2C devices connected to the bus.
 */
void i2c_device_check() {
    u32 j=0;
    for (u8 i=0; i<128; i++) {
        ...
        j=15000;
        while ((!I2C_CheckEvent(I2C2, I2C_EVENT_MASTER_MODE_SELECT)) && j) {
            j--;
        }
        if (!j) {
            printf("\r%d (address=0x%02x) - Error\r", i, i << 1);
        ...
        j=15000;
        while ((!I2C_CheckEvent(I2C2, I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED)) && j) {
            j--;
        }
        if (!j) {
            printf("\r%d (address=0x%02x) - Error\r", i, i << 1);
        ...
        printf("\r%d (address=0x%02x) - OK\r", i, i << 1);
...
/**
 * @brief   Starts the I2C transaction.
 */
...
/**
 * @brief   Stops the I2C transaction.
 */
void i2c_stop() {
    /* Send STOP condition */
    I2C_GenerateSTOP(I2C2, ENABLE);
...
/**
 * @brief   Writes a byte to the I2C bus, waiting for when it will be done.
 */
...
/**
 * @brief   Initializes the TIC120 display.
 */
void tic120_init() {
    i2c_init();
    ...
    i2c_start(TIC120_ADDRESS);

    i2c_write_byte(CON1);
    i2c_write_byte(0x01); // выбор конфигурации , стр01
    i2c_write_byte(CON1);
    i2c_write_byte(0x20); // Enable chip, горизонтальная адресация
    i2c_write_byte(CON1);
    i2c_write_byte(0x09); // выбор страницы
    i2c_write_byte(CON1);
    i2c_write_byte(0x0c); // Display Mode: NORMAL
    i2c_write_byte(CON1);
    i2c_write_byte(0x05); // MUX-rate: 1/34
    i2c_write_byte(CON1);
    i2c_write_byte(0x14); // Bias system: 3
    i2c_write_byte(CON1);
    i2c_write_byte(0x01); // Выбор конфиг . Стр 01
    i2c_write_byte(CON1);
    i2c_write_byte(0x0a); // выбор cтраницы
    i2c_write_byte(CON1);
    i2c_write_byte(0x08);// Фактор умножителя напряжения: 2*V
    i2c_write_byte(CON1);
    i2c_write_byte(0x20); //коэффициент температуры : 0
    i2c_write_byte(CON1);
    i2c_write_byte(0x05); // Vlcd - LOW; Voltage multiplier ON
    i2c_write_byte(CON1);
    i2c_write_byte(200); // Установка напряжения Vlcd
...
/**
 * @brief   Clears the TIC120 display.
 */
void tic120_clear() {
    i2c_start(TIC120_ADDRESS);
    i2c_write_byte(CON2);
    for (u16 cnt = 0; cnt < 640; cnt++) {
        i2c_write_byte(0x0);
    }
    i2c_stop();
}
...
/**
 * @brief   Changes cursor position.
 */
void tic120_set_pos(u8 x, u8 y) {
    i2c_start(TIC120_ADDRESS);

    i2c_write_byte(CON1);
    i2c_write_byte(0x01); // Select command page 00.

    i2c_write_byte(CON1);
    i2c_write_byte(0x40 | y); // Set row: Y = 0 (0...5)

    i2c_write_byte(CON1);
    i2c_write_byte(0x80 | x); // Set row: X = 0 (0...127)
...
/**
 * @brief   Print char in the specified position.
 */
void tic120_write_char_pos(char ch, u8 x, u8 y) {
    tic120_set_pos(x, y);
    tic120_write_char(ch);
}
...
/**
 * @brief   Prints string starting from the specified position.
 */
void tic120_write_string_pos(char *str, u8 x, u8 y) {
    tic120_set_pos(x, y);
    tic120_write_string(str);
}

/**
 * @brief   Prints string with arguments starting from the specified position.
 */
void tic120_printf(char *str, u8 x, u8 y, ...) {
    char buf[32];
    va_list plist;
    va_start(plist, y);
    vsprintf(buf, str, plist);
    va_end(plist);
    tic120_set_pos(x, y);
    tic120_write_string(buf);
}
